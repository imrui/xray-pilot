...
interface FormState {
  ...
  real_name: string
  group_id: string
  expires_at: string  // ISO 日期字符串，空 = 永久
  remark: string
}

const emptyForm = (): FormState => ({
  username: '', real_name: '', group_id: '', expires_at: '', remark: '',
})
...
export default function Users() {
  ...
  const save = useMutation({
    mutationFn: () => {
      ...
      const expiresAt = form.expires_at ? form.expires_at : null
      if (modal.user) {
  ...
  const openEdit = (u: User) => {
    setForm({
      ...
      expires_at: u.expires_at ? u.expires_at.slice(0, 16) : '',  // datetime-local format
      remark: u.remark,
    })
    setErr('')
    ...
    { key: 'id', label: 'ID' },
    {
      ...
      render: (u: User) => (
        <div>
          ...
          {u.real_name && <span className="ml-1.5 text-xs text-slate-400">{u.real_name}</span>}
        </div>
      ),
    },
    {
      key: 'group_name', label: '分组',
      render: (u: User) => u.group_name
        ? <Badge label={u.group_name} variant="blue" />
        : <span className="text-slate-300">—</span>,
    },
    {
      key: 'expires_at', label: '有效期',
      render: (u: User) => {
        if (!u.expires_at) return <span className="text-xs text-slate-400">永久</span>
        const expired = isExpired(u)
        return (
          <span className={`text-xs ${expired ? 'text-red-500' : 'text-slate-600'}`}>
            {expired ? '已过期 · ' : ''}{new Date(u.expires_at).toLocaleDateString()}
          </span>
        )
      },
    },
    ...
    },
    {
      ...
      render: (u: User) => (
        <div className="flex gap-2 flex-wrap">
          <button onClick={() => openEdit(u)} className="text-xs text-slate-500 hover:text-slate-900">编辑</button>
          <button onClick={() => setQrUser(u)} className="text-xs text-indigo-500 hover:text-indigo-700 flex items-center gap-0.5">
            <QrCode className="h-3 w-3" />订阅
          </button>
          <button
            onClick={() => { if (confirm(`重置 ${u.username} 的订阅链接？旧链接将失效`)) resetToken.mutate(u.id) }}
            className="text-xs text-amber-600 hover:text-amber-800 flex items-center gap-0.5"
          >
            <RefreshCw className="h-3 w-3" />重置链接
          </button>
          <button
          ...
          <button
            onClick={() => { if (confirm(`确认删除用户 ${u.username}？`)) remove.mutate(u.id) }}
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">管理订阅用户及其分组 · 共 {data?.total ?? 0} 人</p>
        </div>
      ...
      >
        <div className="space-y-3">
          {!modal.user && (
            <Field label="用户名 *" value={form.username} onChange={f('username')} placeholder="如：alice" />
          )}
      ...
      {qrUser && (
        <QRModal
          ...
          url={qrUser.subscribe_url}
          title={`${qrUser.username} 的订阅`}
        />
      )}
    </div>
  )
